...
def prepare_plot_split(image, mask, predicted_mask, sam_mask, coords, image_path):
...
def make_predictions(model, image_path):
    ...
    with torch.no_grad():
        ...
        predicted_mask = model(image)    # image=[1,1,1024,1024], predicted_mask=[1,1,1024,1024]
        ...
        if len(bboxes) > 1:
            ...
            try:
                ...
                prepare_plot_split(orig_image, orig_mask, predicted_mask, sam_mask, coords, image_path)
            except:
                pass
        else:
            pass

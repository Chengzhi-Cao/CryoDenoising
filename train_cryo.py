...
def main():
    ...
    (_, name2) = os.path.split(args.opt)
    (name,_) = os.path.splitext(name2)
...
#         logger.info(option.dict2str(opt))
    ...
    for epoch in range(start_epoch, total_epochs + 1):
        ...
        for _, train_data in enumerate(train_loader):
            # print('LQ=',train_data['LQ'].shape)
            # print('Event_array=',train_data['Event_array'].shape)
            # print('GT=',train_data['GT'].shape)
            current_step += 1
...
#                     avg_lpips = torch.squeeze(avg_lpips)
                    
                    print('avg_psnr=',avg_psnr)
                    print('avg_ssim=',avg_ssim)
#                     print('avg_lpips=',avg_lpips.data)
                    # log
...
#                     logger.info('# Validation # LPIPS: {:.4e}'.format(avg_lpips))
                    ...
                    write_file('[epoch:{:3d}, iter:{:8,d}'.format(epoch, current_step))
                    write_file('# Validation # PSNR: {:.4e}'.format(avg_psnr))
                    write_file('# Validation # SSIM: {:.4e}'.format(avg_ssim))
#                     write_file('# Validation # LPIPS: {:.4e}'.format(avg_lpips))
